...
%     // ------ correct fix will occur when year is 4-digit in 2le ------
     ...
     d1=findobj('Tag','startedit');
     startd=datevec(get(d1,'string'));
     d2=findobj('Tag','stopedit');
     stopd=datevec(get(d2,'string'));
     ...
     if ((typerun ~= 'v') && (typerun ~= 'c'))
         % ------------- enter start/stop ymd hms values --------------------
           if (typeinput == 'e')
               ...
               s1=findobj('Tag','steptedit');
               deltamin = str2num(get(s1,'string'));
           end;
